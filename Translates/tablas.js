...
ARRNEWLAN.forEach((NEWLANG) => {
  ...
  mRsDetalle.forEach((data) => {
    try {
      ...
      let memo_translate = translate(data.tab_memo, NEWLANG).toString();
      data.tab_memo = memo_translate[0].toUpperCase() + memo_translate.slice(1);

      if (data.col_desc) {
        let desc_translate = translate(data.tab_desc, NEWLANG);
      ...
      if (data.tab_info) {
        let info_translate = translate(data.tab_info, NEWLANG);
